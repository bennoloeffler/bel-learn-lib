(ns inspect
  ;; extracted from https://github.com/Ivana-/bb-debug
...
(def gui-settings
  {:colors {:lite-brown (Color. 156 93 39)
   ...
   :fonts {:tree (Font. "Monospaced" Font/PLAIN font-size)}
...
(defrecord TreeNodeUserObjectType [component path value]
  java.lang.Object
  (toString [_] (str (when (seq path) (pprint-safe path))
                     "\n"
                     (pprint-safe value))))

(defn- label ^JLabel [^String text & [{:keys [color]}]]
...
(defn- show [x]
  (cond
    (some #(instance? % x) [clojure.lang.ChunkedCons])
    (label (str "#object[" (.. ^java.lang.Object x getClass getName) "]"))

    (map-entry? x)
    (wrap-in-panel (show (key x)) (show (val x)))

    (vector? x)
    (label (str "["  (count x) "]") {:color :lite-grey})
    ...
    (associative? x)
    (label (str "{"  (count x) "}") {:color :lite-grey}) ;; map? or datomic entity

    (coll? x)
    (label (str "(" (let [bc (bounded-count (inc max-seq-size) x)]
                      (if (> bc max-seq-size)
                        (str ">" max-seq-size)
                        bc)) ")")
           {:color :lite-grey})

    (instance? clojure.lang.IExceptionInfo x)
    (label (str "#ex-info[" (ex-message x) " " (ex-data x) "]"))

    (instance? java.lang.Throwable x)
    (label (str (.. ^java.lang.Object x getClass getSimpleName) " " (ex-message x)))

    (some #(% x) [nil? boolean? number? keyword?])
    (label (pr-str x) {:color :lite-brown})

    (string? x)
    (label (pr-str x) {:color :green})
...
(defn- ->node ^DefaultMutableTreeNode [component path value]
...
(defn- my-tree-cell-renderer ^DefaultTreeCellRenderer []
...
(defn- jframe-show [^JFrame jframe data & [title]]
  (jframe-update-content jframe (obj->jtree data) title))
